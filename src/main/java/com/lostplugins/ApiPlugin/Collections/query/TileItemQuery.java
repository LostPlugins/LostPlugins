package com.lostplugins.ApiPlugin.Collections.query;

import com.lostplugins.ApiPlugin.ApiPlugin;
import com.lostplugins.ApiPlugin.Collections.ETileItem;
...
public class TileItemQuery {
    ...
    public TileItemQuery withName(String name) {
        ...
        {
            try {
                return ApiPlugin.itemDefs.get(tileItem.tileItem.getId()).getName().equals(name);
    ...
    public TileItemQuery nameContains(String name) {
        ...
        {
            try {
                return ApiPlugin.itemDefs.get(tileItem.tileItem.getId()).getName().contains(name);
            } catch (ExecutionException e) {
                throw new RuntimeException(e);
            }
        }).collect(Collectors.toList());
                        ...
                        filter(item ->
                        {
                            try {
                                return WildcardMatcher.matches(input.toLowerCase(),
                                        Text.removeTags(ApiPlugin.itemDefs.get(item.tileItem.getId()).getName().toLowerCase()));
                            } catch (ExecutionException e) {
                                throw new RuntimeException(e);
                            }
                        }).
    ...
    public boolean isNoted(ETileItem item) {
        ItemComposition itemComposition = ApiPlugin.itemDefs.get(item.tileItem.getId());
